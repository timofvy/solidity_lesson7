...
const fee = 500
const reserve0 = ethers.utils.parseEther("10000").toBigInt()
const reserve1 = ethers.utils.parseEther("20000").toBigInt()

const amount0ToMint = ethers.utils.parseEther("200000").toBigInt()
const amount1ToMint = ethers.utils.parseEther("100000").toBigInt()

const MIN_TICK = -885000
const MAX_TICK = 885000
// SWAP_input
const amountIn = ethers.utils.parseEther("1").toBigInt()
const amountOutMinimum = ethers.utils.parseEther("0").toBigInt()
// SWAP_output
const amountOut = ethers.utils.parseEther("1").toBigInt()
...
describe("UniswapV3AdapterContract unit tests", function () {
    ...
    it("should successful swap Input", async function () {
        const { erc20Tim, erc20Sv, uniswapV3Adapter, token0, token1 } = await loadFixture(deployFixture);
        await erc20Tim.approve(uniswapV3Adapter.address, '111111111111111111111111111111111111111111');
        await erc20Sv.approve(uniswapV3Adapter.address, '1111111111111111111111111111111111111111111');
        await uniswapV3Adapter.createPool(token0, token1, fee, reserve0, reserve1);
        await uniswapV3Adapter.mintNewPosition(token0, token1, fee, reserve0, reserve1, MIN_TICK, MAX_TICK);
    ...
    //     const { erc20Tim, erc20Sv, uniswapV3Adapter, token0, token1 } = await loadFixture(deployFixture);
    //     await uniswapV3Adapter.createPool(token0, token1, fee, reserve0, reserve1);
    //     console.log(1111111, reserve0, reserve1);
    //     await erc20Tim.approve(uniswapV3Adapter.address, amount0ToMint);
    //     await erc20Sv.approve(uniswapV3Adapter.address, amount1ToMint);
    ...
    //     )
    //     expect(eventCollect[0].args.amount0).to.equal(0);
    //     expect(eventCollect[0].args.amount1).to.equal(0); // >0
    // })
